...
const vec3 sunColor = vec3(.95, 0.95, 0.9);
...
vec3 sunAndSky(float rayAngle) {
    ...
    float sunIntensity = smoothstep(4.0, 0.0, angleToSun);
...
vec4 effect(vec4 c, Image t, vec2 tc, vec2 sc) {
    ...
    //Clamp the radiance to avoid overexposure
